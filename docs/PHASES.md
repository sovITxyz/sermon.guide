...
- [x] Phase 45 — Google Docs link/pull/unlink (code-complete on branch phase-45/google-docs-link, PR #N; spike SETTLED — native-Doc conversion viable, pull via markdown export with http:///read→/read normalization, no fallback; all keyless gates green, ⏳ merge gated on live link/pull/unlink verification against the connected Google account; finalized at merge. **Migration 0010** (`editor_links`, additive off 0009, single head): one Google-Doc link per document via a partial UNIQUE index `(document_id) WHERE state='linked'`, per-user index, both FKs ON DELETE CASCADE, `state`/`linked_at` now() defaults; up→down→up clean, zero `alembic check` drift. **Drive client** (`api/drive_client.py`): decrypted access/refresh tokens stay in-memory only (Bearer header + token-refresh POST body — never a log/metric/Sentry/response/browser line; DB holds only AES-256-GCM ciphertext); on `invalid_grant` it refreshes via a fixed module-constant Google endpoint and persists the fresh access-token ciphertext scoped by `(id,user_id)`; all Google endpoints are module constants, the only dynamic URL part is the file id percent-encoded with `safe=''` (SSRF-closed), mimeType is the fixed `text/markdown`/native-doc constant. **link/status/pull/unlink** (`api/editor_links.py`): every `editor_links`/`oauth_connections` statement triply-scoped to the JWT `current_user.user_id` (+document_id+state), `_require_owned_document` gates FIRST (non-UUID/nonexistent/cross-tenant/soft-deleted → byte-identical 404, `_no_linked_row` shares the identical 404 shape — no existence oracle), `provider_file_id` only ever read from the user's own row or a fresh upload id (never the request), `UnlinkRequest` is `extra=forbid`→422. **Snapshot-before-overwrite:** `_run_pull_overwrite` inserts the PRIOR content/content_text/schema_version into `sermon_doc_revisions` (source='pull') BEFORE the overwrite under a single commit covering snapshot+overwrite+cursor bump — a crash cannot lose the prior manuscript; `content_text` is server re-derived via `derive_content_text`, Google's markdown is never trusted. **Remote version** is an opaque cursor (str-normalized, equality-only — never parsed/ordered/eval'd); `normalize_read_hrefs` only strips an `http(s)://` prefix whose host is empty/loopback/dummy immediately followed by `/read/`, and `parseReadHref` rejects absolute/protocol-relative/nested-slash/traversal hrefs — only a clean same-origin single-segment `/read/<id>` mints a citation (Phase 43 attacker-docx defense mirrored). **Web:** editor read-only lock + "Editing externally" banner while linked, link/status/pull/unlink proxies forwarding the bearer server-side with a structural `{mode}`-only whitelist; `web_url` rendered `target=_blank rel="noopener noreferrer"` behind an https-only `safeWebUrl` guard, never a server-side fetch/redirect target. **Security gate: PASS** — all five invariants (token-in-memory-only, full tenant scoping, snapshot-before-overwrite, SSRF-closed, opaque remote version) verified against the real code; one low-severity robustness nit (a redundant post-pull `get_version` outside try/except that can 500 instead of cleanly degrade — no data loss, no token leak, not security-blocking). **Keyless gates:** api 378 passed (incl. 22 editor_links unit), web 346 passed across 27 files (editor-links whitelist + routes + e2e on fake-api), migration 0010 round-trips clean on dev Postgres, isolation 3/3. **live link/pull/unlink DEFERRED to the operator checkpoint** (needs the live Google token); finalized at merge.)
- [ ] Phase 46 — Microsoft Graph provider (⛔ operator: Azure creds first)
- [x] Phase 47 — Schedule a sermon from the editor (completed 2026-06-29, branch `phase-47/schedule-from-sermon`, PR #83 — the reverse entry point for B3 sermon scheduling: calendar↔sermon linking already existed but was **calendar-first only** (start on `/calendar`, create/pick an event, attach a sermon); this adds a **📅 Schedule** toolbar action in `SermonEditor` that creates a calendar event already linked to the open sermon in **one POST**. **No backend schema change** — no migration, no new env. The create-event whitelist (`lib/calendar.ts:whitelistCreateEvent`) + the `CalendarEventCreate` type + the `/api/sermon-events` POST proxy now **forward `document_id`** (was deferred to a separate PATCH in Phase 41); safe because the API ownership-checks a non-null `document_id` and returns a no-oracle 404, so the proxy forwards it verbatim with no scope-widening or tenant leak (the editor only ever sends its own already-owned doc id). New `components/ScheduleSermonPopover.tsx` reuses the calendar `DialogShell` (date defaults to `today()`, event title prefilled from the live sermon title, optional series); on success an inline `role="status"` confirmation deep-links to `/calendar?view=month&date=<event_date>`. **ZERO `dangerouslySetInnerHTML`** — all inputs are controlled React state rendered as text nodes. **Gates:** web 367 passed (inverted the `calendar-body` whitelist `document_id` cases to forwards-string/omits-absent/rejects-non-string + new `ScheduleSermonPopover` tests; existing `SermonEditor` suite unaffected), biome + `tsc --noEmit` clean, Playwright `schedule-from-sermon.spec.ts` 2/2 (editor→Schedule→calendar round-trip asserts the event is **LINKED** — its chip click navigates back to the manuscript — and a series entered in the popover persists; the fake-api now accepts + ownership-checks `document_id` on `POST /calendar/events`). First slice of the scoped-search / shared-scope / scheduling plan; the optional `SermonList` per-row schedule action was intentionally deferred to keep the PR atomic.)
- [x] Phase 48 — Library collections (completed 2026-06-29, branch `feature/scoped-search-suite` — folders to organize uploaded books, wiring up the dormant `collections` table. New `collection_books` membership table (migration **0011**, down_revision 0010): collection_id/book_id/user_id FKs ON DELETE CASCADE, denormalized `user_id` (EditorLink/SermonDocRevision precedent), `UniqueConstraint(collection_id, book_id)` + `Index(user_id, book_id)`. New `api/collections_routes.py` mirroring calendar_routes.py: module-level `_xxx_stmt` builders (compile-pinned tenant seam), `extra="forbid"`, no-oracle 404; routes POST/GET `/collections`, GET/PATCH/DELETE `/collections/{id}`, POST/DELETE `/collections/{id}/books` (add-books INTERSECTS requested book_ids with the owner's `user_library` — can only shrink, never widen — + ON CONFLICT DO NOTHING). Exports `_owned_collection_ids_stmt`/`_member_book_ids_stmt` for Phase 49. Web: Collection types, `lib/collections.ts` whitelists, 3 same-origin proxies, `getCollections()`, `CollectionsPanel` wired into `/library`. Gates: api collections unit (tenant pins, clamp, ON CONFLICT idempotency, smuggled user_id→422), web vitest, biome+tsc; tenant-auditor + schema-reviewer PASS.)
- [x] Phase 49 — Scoped search over chosen books + collections (completed 2026-06-29, branch `feature/scoped-search-suite` — the headline. `SearchRequest`/`SummaryRequest` gain optional `book_ids`/`collection_ids` (None = whole library; `extra="forbid"` kept). `run_search` resolves the JWT library then **INTERSECTS**: `effective = (requested book_ids ∪ owned-collection member book_ids) & library` — can only SHRINK, never widen; collection_ids ownership-checked (cross-tenant → no-oracle 404); empty intersection short-circuits to empty BEFORE any embed/Milvus call; `sorted(effective)` to both arms. Web: whitelists forward the arrays (omit = whole library), shared `SelectionProvider` (sessionStorage-bridged across `/library` + `/search`), `LibraryBrowser` checkboxes + `SelectionBar` + scope label, `SearchPanel`/`LibraryDrawer` send the resolved scope. The old "no book_ids" docstring was revised — the tenant invariant is preserved by the intersection. Gates: api scoped-search/summary unit (widening clamped to the library, empty→empty no-arm-run, cross-tenant collection_id→404), **golden tenant-isolation 3/3**, web vitest + e2e (scoped book_ids carried into the summary POST); **tenant-auditor PASS**. Fix during e2e: a `SelectionProvider` hydration race that dropped the selection crossing `/library`→`/search` (the persist effect clobbered sessionStorage on mount) — gated persist on a `hydrated` flag.)
- [x] Phase 50 — Per-sermon citation scope (completed 2026-06-29, branch `feature/scoped-search-suite` — the sermon writer "on the same page". `documents` gains `scope_book_ids`/`scope_collection_ids` JSONB NOT NULL server_default '[]' (migration **0012**, down_revision 0011 — add_column backfills existing rows without a rewrite). `api/documents.py` round-trips + clamps the scope (the `_update_stmt` RETURNING includes both columns). Web: an editor "Scope" control persists via the EXISTING autosave PATCH path (`sermon-autosave.ts` buildPatchBody/EditorSnapshot/isDirty extended, reusing the single-flight + 409 machinery); the `LibraryDrawer` citation search is scoped to the sermon's books, and stale `scope_collection_ids` are pruned on load. Gates: api documents unit (scope round-trip + RETURNING), web vitest + e2e (scoped citation); verify + schema-reviewer PASS.)
- [x] Phase 51 — Search history ("Recent" panel, full-result replay) (completed 2026-06-29, branch `feature/scoped-search-suite` — past summary searches saved + instantly replayable (the `/search-summary` pipeline is 2–4 min + an LLM call). New `search_history` table (migration **0013**, down_revision 0012): user_id FK CASCADE, `query`, `scope_book_ids`/`scope_collection_ids` JSONB, `result` JSONB (the full SummaryResponse), `created_at` (immutable, no onupdate), `Index(user_id, created_at DESC)`. `api/summary.py` records each successful summary **BEST-EFFORT** (a history-write failure can never fail the already-computed summary) with a per-user retention cap. New `api/search_history.py` (prefix `/search-history`, `_xxx_stmt` + no-oracle 404): GET list (lightweight preview, not the full citations blob), GET `/{id}` (full replay blob), DELETE `/{id}`, DELETE all. Web: proxies + types + a `RecentSearches` panel beside `SearchPanel` on `/search` — reopening HYDRATES the saved summary+citations with NO second `/search-summary` call. Gates: api history unit (tenant pins, save-on-summary inserts one row, retention prune, best-effort), web vitest + e2e (save→reopen-without-rerun→delete); verify PASS. Fix during e2e: a brittle open-row locator.)
- [x] Phase 55 — Collection scope picker on /search (completed 2026-07-01, branch `phase-55/search-collection-picker` — a PURE front-end affordance over the Phase 49 scoped-search plumbing: let the searcher scope by whole COLLECTION from `/search` itself, not only by ad-hoc books ticked over on `/library`. New `components/search/CollectionScopePicker.tsx` — a disclosure button (`data-testid="collection-scope-picker"`, `aria-expanded`) opening a checkbox list, one per collection (`checked` = the shared selection includes it, `onChange` = `toggleCollection`, label = name + book count) plus a "Clear". It is driven ENTIRELY through the shared `useSelection()` provider (mirrors the Collections block of `SermonEditor`'s `ScopePopover` for markup + a11y, but wired to the shared provider, not per-sermon callbacks), so a collection ticked here also shows selected on `/library` and "Clear" resets both. The user's `collections` are threaded page → `SearchWorkspace` → `SearchPanel` as a prop (no new fetch); `SearchPanel` already folds `collection_ids` into the `/search-summary` POST (Phase 49), so an empty selection is unchanged (= whole library). **NO backend / proxy / migration change** — `api/search.py` / `api/summary.py` already accept + ownership-check + intersect `collection_ids` (cap 500, no-oracle 404), and `lib/summary.ts:whitelistSummary` already forwards it; the tenant scope surface is untouched. Gates: web vitest (a new `SearchPanel` spec ticks the picker and asserts the POST carries `collection_ids` + the scope label updates, unticking clears it), biome + `tsc --noEmit` clean, Playwright `search.spec.ts` (open picker on `/search` → tick a collection → assert `search-scope` + the POST `collection_ids`). Deviation from the brief: the e2e `fake-api` served an EMPTY `/collections` list, so a single collection ("Grace Collection", member = On Grace) was seeded there — the only way to have a collection to tick; verified it keeps the Phase 49 book-scope e2e locators unambiguous.)
